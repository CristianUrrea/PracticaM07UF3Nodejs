...
module.exports.getMediaListTipus = async (serviceData) => {
    const responseObj = constants.responseObj;
    try{
        const data = {
            query: {
              tipus: serviceData.tipus
            },
            model: Media,
            projection: {
                '__v': false
        ...
        console.log('ERROR-Service-getMediaListTipus: ', err);
    }
    return responseObj;
}
...
module.exports.updateMedia = async (serviceData) => {
    ...
    try{
        ...
        if(serviceData.tipus) data.updateQuery.tipus = serviceData.tipus;
